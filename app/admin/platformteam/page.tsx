  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Équipe plateforme / Platform Team</h1>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-sm border border-orange-100 p-6 mb-6">
          <h2 className="font-bold text-gray-900 mb-4">Nouveau membre / New member</h2>
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-500 font-medium mb-1.5">Nom / Name *</label>
            ...
            <div>
              <label className="block text-xs text-gray-500 font-medium mb-1.5">Mot de passe / Password *</label>
              <input type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
                placeholder="Min. 8 caractères" className={INPUT} required minLength={8} />
            </div>
            <div>
              <label className="block text-xs text-gray-500 font-medium mb-1.5">Rôle / Role *</label>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {team.map((member, idx) => (
            <div
              key={member.id}
              className={`flex items-center gap-4 px-5 py-4 ${idx < team.length - 1 ? 'border-b border-gray-50' : ''}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {member.status === 'suspended' && (
                    <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Suspendu</span>
                  )}
                </div>
                <p className="text-xs text-gray-400">{member.email}</p>
                <p className="text-xs text-gray-400">Ajouté le {new Date(member.created_at).toLocaleDateString('fr-FR')}</p>
              </div>
              ...
              {member.id === currentAdminId && (
                <span className="text-xs text-gray-400 italic">Vous / You</span>
              )}
            </div>
          ))}
        </div>
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${map[role] ?? 'bg-gray-100 text-gray-500'}`}>
      {role.replace('_', ' ')}
    </span>
  )
}
